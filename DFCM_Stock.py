...
configure['InputFilePath'] = './2-Stock/2plus-supervisedDataSet_zscore.csv'
configure['OutputFilePath'] = './2-Stock/6-DFCM.csv'
configure['PltFilePath'] = './2-Stock/6-DFCM/'
configure['AllAttributes'] = 10
configure['TargetAttributes'] = 1
configure['InputAttributes'] = [1,2,3,4,5,6,7,8,9]
configure['OutputAttributes'] = [11]
configure['TimeAttributes'] = [0]
configure['Length'] = 535

configure['global_epochs'] = 10

configure['f_batch_size'] = 10
configure['f_epochs'] = 15
configure['hidden_layer'] = 10

configure['n_batch_size'] = 10
...
for i in range(len(configure['OutputAttributes'])):
    ...
    model_f[i].add(Dense(hidden_layer, input_dim=train_X.shape[1], activation='relu', use_bias=True))
    model_f[i].add(Dense(hidden_layer, input_dim=hidden_layer, activation='relu', use_bias=True))
    model_f[i].add(Dense(hidden_layer, input_dim=hidden_layer, activation='relu', use_bias=True))
    model_f[i].add(Dense(1, input_dim=hidden_layer, use_bias=True))
